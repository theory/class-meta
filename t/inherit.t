...
BEGIN {
    ...
    ok( $c->add_meth(name => 'foo'), "Add foo method to One" );
    ok( $c->add_meth(name => 'bar'), "Add bar method to One" );
...
BEGIN {
    ...
    ok( $c->add_ctor(name => 'two_new'), "Create Two's ctor" );

    # Add an attribute.
    ok( $c->add_attr( name     => 'description',
                      view     => Class::Meta::PUBLIC,
                      authz    => Class::Meta::RDWR,
                      create   => Class::Meta::GETSET,
                      type     => 'string',
    ...
    # Add a method.
    ok( $c->add_meth(name => 'woah'), "Add woah method to One" );
    # Add an overriding method.
    ok( $c->add_meth(name => 'bar'), "Add bar method to Two" );
...
is( $one_attrs[0]->call_get($one), undef, "Check attr call id" );
ok( $one_attrs[1]->call_set($one, 'howdy'), "Call set on One" );
is( $one_attrs[1]->call_get($one), 'howdy', "Call get on One" );
...
is( $two_attrs[0]->call_get($two), undef, "Check attr call id" );
is( $two_attrs[1]->call_get($two), 'hello', "Call get on Two" );
ok( $two_attrs[1]->call_set($two, 'howdy'), "Call set on Two" );
is( $two_attrs[1]->call_get($two), 'howdy', "Call get on Two again" );
is( $two_attrs[2]->call_get($two), 'yello', "Call get on Two" );
ok( $two_attrs[2]->call_set($two, 'rowdy'), "Call set on Two" );
is( $two_attrs[2]->call_get($two), 'rowdy', "Call get on Two again" );
