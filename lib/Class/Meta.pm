...
# $Id: Meta.pm,v 1.17 2003/11/19 03:57:46 david Exp $
...
##############################################################################
{
    ...
    sub new {
        ...
        # Return!
        return bless { pkg => $p{class} }, ref $pkg || $pkg;
    }
...
# build()
    sub build {
        my $self = shift;
        my $spec = $classes{ $self->{pkg} };
        no strict 'refs';

        # Build the attribute methods.
        foreach my $attr (@{$spec->{build_attr_ord}}) {
        }

        # Build the constructors.
        foreach my $ctor (@{$spec->{build_ctor_ord}}) {
            # Create a constructor.
            *{$spec->{pkg} . '::' . $ctor->my_name } = sub {
                my $class = shift;
                my $init = {@_};
                my $new = bless({}, ref $_[0] || $_[0]);

                foreach my $pobj (@{ $spec->{attrs} }) {
                    my $p = $pobj->my_name;
                    if (exists $init->{$p}) {
                        # Assign the value passed in.
                        Carp::croak("Write access to attribute '$p' denied")
                          unless $pobj->my_view > READ;
                        $pobj->set($new, delete $init->{$p});
                    } else {
                        # NOTE: Might have to construct a new object here.
                        $new->{$p} = $pobj->my_default;
                    }
                }
                if (my @attrs = keys %$init) {
                    ...
                    local $" = "', '";
                    Carp::croak("No such $c '@attrs' in $spec->{pkg} "
                                . "objects");
                }
                return $new;
            };
