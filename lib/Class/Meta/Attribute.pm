...
# $Id: Attribute.pm,v 1.17 2004/01/08 07:02:27 david Exp $
...
=head1 SYNOPSIS
  ...
  print "\nAttributes:\n";
  for my $attr ($class->my_attrs) {
      print "  o ", $attr->my_name, " => ", $attr->call_get($thingy) $/;
      if ($attr->authz >= Class::Meta::SET && $attr->my_type eq 'string') {
...
Class::Meta-generated class, simply call its C<my_class> method to retreive
its Class::Meta::Class object, and then call the C<my_attrs()> method on
...
=head3 my_name
  ...
  my $name = $attr->my_name;

Returns the name of the attribute.

=head3 my_type

  my $type = $attr->my_type;
...
=head3 my_desc
  ...
  my $desc = $attr->my_desc;

Returns a description of the attribute.

=head3 my_label

  my $label = $attr->my_label;
...
C<my_default()> may change on subsequent calls. It all depends on what was
